...
def train(args):
    ...
    # Initialize model
    if args.pretrained:
        model = fcn_resnet50(pretrained_backbone=True)
        # Change the final layer to match the number of classes
        model.classifier[4] = torch.nn.Conv2d(512, train_data.classes_seg, kernel_size=1)
    else:
        model = fcn_resnet50(pretrained=False, num_classes=train_data.classes_seg)

    model = DeepSegmenter(model, num_classes=train_data.classes_seg)
